...
def print_board(board):
    min_col = min(board, key=lambda k: k[0])[0]
    max_col = max(board, key=lambda k: k[0])[0]
    min_row = max(board, key=lambda k: k[1])[1]
    max_row = min(board, key=lambda k: k[1])[1]

    for row in range(min_row, max_row - 1, -1):
        for col in range(min_col, max_col + 1):
            val = board.get((col, row), '0')
            val = '#' if val == '1' else ' '
            print(val, end='')
        print()
